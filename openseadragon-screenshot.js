  ...
  $.extend(
    ...
    /** @lends OpenSeadragon.ControlDock.prototype */ {
      ...
      takeScreenshot: function () {
        ...
        var downloadFunction = function () {
          ...
          if (viewer.screenshotInstance.showOptions && viewer.scalebarInstance && document.getElementById('screenshotDrawScalebar').checked) {
            ...
            if (typeof screenshotDrawArrows !== 'undefined' && viewer.annotationInstance) viewer.annotationInstance.drawArrowsOnCanvas(Canvas);

            var scalebarCanvas = viewer.scalebarInstance.getAsCanvas();
            var location = viewer.scalebarInstance.getScalebarLocation();

            // Modified by EK
            // On Retina devices, canvas width does not equal to container width.
            // (Currently it is twice bigger.)  So scalebarCanvas is scaled up by
            // factor of "viewer.drawer.canvas.width / viewer.viewport.containerSize.x".
            const factor = imgCanvas.width / originalCSx;
            ctx.drawImage(scalebarCanvas, location.x * factor, location.y * factor, scalebarCanvas.width * factor, scalebarCanvas.height * factor);
          } else {
        ...
        if (!this.showOptions) {
          ...
          requestAnimationFrame(function () {
            // NEeded for HTML/JS to find out the viewport just resized
            viewer.forceRedraw();
            if (viewer.world.getItemAt(0).getFullyLoaded()) {
              downloadFunction();
            } else {
      ...
      menuUpdate: function () {
        ...
        document.getElementById('screenshotTextMessage').innerHTML =
          'Size of download: ' + Math.round(this.screenshotWidth) + 'x' + Math.round(this.screenshotHeight);
      },

      // takeScreenshotWithPredefinedOptions is added by EK
      takeScreenshotWithPredefinedOptions: function () {
        // Change menu options and update menu.

        const drawScalebar = document.getElementById('screenshotDrawScalebar').hasAttribute('checked');
        if (!drawScalebar) {
          document.getElementById('screenshotDrawScalebar').setAttribute('checked', true);
        }
        const useViewportSize = document.getElementById('screenshotUseVpSize').checked; // radio
        const zoomFactorCheck = document.getElementById('screenshotZFCheck').checked; // radio
        document.getElementById('screenshotUseVpSize').checked = true;
        document.getElementById('screenshotZFCheck').checked = false;
        this.menuUpdate();

        // Change screenshot options and take screenshot.

        const showOptions = this.viewer.screenshotInstance.showOptions;
        const backgroundColor = this.viewer.scalebarInstance.backgroundColor;
        this.viewer.screenshotInstance.showOptions = true;
        this.viewer.scalebarInstance.backgroundColor = backgroundColor == 'none' ? 'transparent' : backgroundColor;
        this.takeScreenshot();

        // Restore previous options.

        document.getElementById('screenshotUseVpSize').checked = useViewportSize;
        document.getElementById('screenshotZFCheck').checked = zoomFactorCheck;
        this.menuUpdate();

        // There is a problem for restoring backgroundColor, showOptions, and screenshotDrawScalebar options
        // to the previous values: takeScreenshot() has an async operation for loading image.
        // They should be restored to the previous values *after* downloadFunction has returned.
        // Now what would you do?

        //this.viewer.scalebarInstance.backgroundColor = backgroundColor;
        //this.viewer.screenshotInstance.showOptions = showOptions;
        //if (!drawScalebar) {
        //  document.getElementById("screenshotDrawScalebar").removeAttribute('checked');
        //}
      },
    }
  );
